...
и возвращает ответ в виде вектора единиц и нулей. Список кандидатов дан в виде вектора строк. 
Если слово есть во множестве, в ответе будет единица, иначе — ноль. В коде вы видите библиотеку <chrono>. 
...
vector<int> CountNames(const set<string>& storage, const vector<string>& candidates)
{
    ...
    for (auto& name : candidates)
    {
        output.push_back(storage.count(name)); //here
    }
