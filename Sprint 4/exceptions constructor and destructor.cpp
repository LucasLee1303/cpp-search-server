...
Доработайте конструктор класса Rational так, чтобы при попытке сконструировать дробь с нулевым знаменателем, выбрасывалось исключение типа domain_error.
int main() {
    try {
...
Добавьте проверку знаменателя на равенство нулю с выбрасыванием соответствующего исключения в параметризованный конструктор, 
принимающий числитель и знаменатель дроби. Это сделает невозможным конструирование дроби с нулевым знаменателем.
*/
#include <iostream>
#include <numeric>
#include <stdexcept>
...
public:
    ...
    Rational(int numerator, int denominator)
        ...
        if (denominator_ == 0) throw domain_error ("denominator = 0"); //here
        Normalize();
    }
