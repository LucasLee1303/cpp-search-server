/*
Задание
Замените способ настройки стоп-слов в классе SearchServer. Вместо метода SetStopWords разработайте параметризованные конструкторы, 
принимающие стоп-слова в следующих форматах:
...
в виде произвольной коллекции строк, такой как vector или set. Пустые строки и слова-дубликаты внутри коллекции должны игнорироваться. 
Чтобы не дублировать код, примените шаблоны.
...
Конструктор, как и обычная функция, бывает шаблонным. Шаблонный конструктор может принимать коллекцию стоп-слов, 
находящихся в разных коллекциях — в вашем случае, в vector и set:
class SearchServer {
public:
...
Доработайте функцию SplitIntoWords: исключите из неё возврат пустых слов. Реализуйте параметризованный конструктор, 
который принимает строку со стоп-словами.
*/
