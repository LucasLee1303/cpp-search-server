/*Задание
Начнём избавляться от пар в коде поисковой системы. Вам понадобятся функции из урока«Ранжирование по релевантности» из предыдущей темы.
Объявите структуру DocumentContent так же, как в уроке. Перепишите на структуру DocumentContent функции AddDocument, MatchDocument, FindAllDocuments и FindTopDocuments. Теперь они будут принимать вектор DocumentContent вместо вектора пар.
...
Замените вектор пар {id, вектор слов} на вектор DocumentContent — vector<DocumentContent>. Важен порядок полей при добавлении документов в вектор: сначала id, 
потом вектор слов!*/
