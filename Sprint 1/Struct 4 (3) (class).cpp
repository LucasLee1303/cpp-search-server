/*Для добавления документов в поисковый индекс есть метод AddDocument. Чтобы задать стоп-слова, превратите функцию ParseStopWords в метод SetStopWords. 
Он всё так же примет на вход строку с разделёнными пробелом словами, но не будет создавать и возвращать множество, а сразу добавит эти слова в соответствующее 
поле класса.
...
Перенесите функцию ParseStopWords внутрь класса (под public:!) и переименуйте в SetStopWords. Полученные стоп-слова добавляйте сразу во множество stop_words_. 
Для этого используйте метод insert.*/
#include <algorithm>
#include <iostream>
#include <set>
#include <string>
