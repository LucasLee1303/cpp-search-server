/*Задание
Убедимся, что полученная шаблонная функция может иметь и более сложный параметр-тип. Примените шаблонную функцию ComputeTermFreqs и определите, 
какое животное встречается наибольшее число раз. Животное в этом задании задаётся парой pair<string, int>, содержащей имя и возраст.
...
Вызовите ComputeTermFreqs для вектора animals, проитерируйтесь по результату и найдите ключ с наибольшим значением. Если очередной ключ имеет частоту, 
равную уже найденной максимальной, не переписывайте им ключ-ответ.
*/
#include <iostream>
#include <map>
#include <string>
...
pair<string, int> FindMaxFreqAnimal(const vector<pair<string, int>>& animals) {
	...
	auto frequencies = ComputeTermFreqs(animals);
	auto max = max_element(frequencies.begin(), frequencies.end(),
