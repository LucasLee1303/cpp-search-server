/*
Задание 2
Большинство людей интересуется только актуальной информацией. Поэтому неясно, зачем просить пользователя поисковой системы каждый раз передавать статус. 
Научите метод FindTopDocuments обходиться одним аргументом — поисковым запросом. То есть поиск должен производиться только по актуальным документам.
Этого можно добиться, указав для параметра-статуса значение по умолчанию:
...
Достаточно добавить = DocumentStatus::ACTUAL сразу после второго параметра метода FindTopDocuments.
*/
